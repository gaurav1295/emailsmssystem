...
if (isset($_POST['submit'])){
    ...
    $name_sg=strtoupper($_POST['name_sg']);
    $agent_sg=strtoupper($_POST['agent_sg']);
    $pnr_g=strtoupper($_POST['pnr_g']);
    ...
    if(isset($_POST['temp_check'])){
        ...
        $message="Thanks For Booking With Us.Your Confirmation Number For Your Flights Is ".$pnr_g.".For Any Assistance Regarding Your Booking, Fell Free To Call Us on 18004836015.";
     ...
     if($pnr_g==''){
        conf($self,'e','Please Enter pnr number, or please match the size , it should be six alphanumeric long');
    }else
    if (strlen($pnr_g)<6) {
        conf($self,'e','less than 6');
    }
    else
    if (strlen($pnr_g)>6) {
        conf($self,'e','greater than 6');
    }
    ...
    if($sms_count>=$sms_charge){
        ...
        $isms->send_by=$cid;
        $isms->name_sg=$name_sg;
        $isms->agent_sg=$agent_sg;
        $isms->pnr_g=$pnr_g;
        $isms->save();
