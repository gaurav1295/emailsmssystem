...
if (isset($_POST['submit'])){
    ...
    $name_g=strtoupper($_POST['name_g']);
    $pnr_g=strtoupper($_POST['pnr_g']);
    $agent_name_g=strtoupper($_POST['agent_name_g']);

    if(isset($_POST['temp_check'])){

        $message=file_get_contents('email_temp_one.php');
        $message = str_replace('%custname%', $name_g, $message); 
        $message = str_replace('%custpnr%', $pnr_g, $message); 
    }else{
        if($message==''){
    ...
    if($name_g==''){
        conf($self,'e','Please Enter Customer name');
    }
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $imail->name_g=$name_g;
        $imail->pnr_g=$pnr_g;
        $imail->agent_name_g=$agent_name_g;
        $imail->save();
    }
